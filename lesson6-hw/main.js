...
// --написати пошук всіх об'єктів, в який в modules є sass
// --написати пошук всіх об'єктів, в який в modules є docker
let scan = coursesArray.reduce(function (accumulator, item) {
    for (index=0; index < item.modules.length; index++) {

        if (item.modules[index] === 'sass') {
            accumulator[0].push(item);
        } else if (item.modules[index] === 'docker') {
